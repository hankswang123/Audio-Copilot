import { ZPRealtimeEventHandler } from './event_handler.js';
import { ZPRealtimeAPI } from './api.js';
import { ZPRealtimeConversation } from './conversation.js';
import { ZPRealtimeUtils } from './utils.js';
...
/**
 * @typedef {FormattedItemType & BaseItemType} ZPItemType
 */
 ...
 * @class
 */
export class ZPRealtimeClient extends ZPRealtimeEventHandler {
  /**
  ...
  constructor({ url, apiKey, dangerouslyAllowAPIKeyInBrowser, debug } = {}) {
    ...
    this.realtime = new ZPRealtimeAPI({
      url,
      apiKey,
      dangerouslyAllowAPIKeyInBrowser,
      debug,
    });
    this.conversation = new ZPRealtimeConversation();
    this._resetConfig();
      ...
      for (const c of content) {
        if (c.type === 'input_audio') {
          if (c.audio instanceof ArrayBuffer || c.audio instanceof Int16Array) {
            c.audio = ZPRealtimeUtils.arrayBufferToBase64(c.audio);
          }
        }
      }
      this.realtime.send('conversation.item.create', {
        item: {
          type: 'message',
          object: 'realtime.item', // Added by ZhipuAI
          role: 'user',
          content,
        },
      });
  ...
  appendInputAudio(arrayBuffer) {
    if (arrayBuffer.byteLength > 0) {
      this.realtime.send('input_audio_buffer.append', {
        audio: ZPRealtimeUtils.arrayBufferToBase64(arrayBuffer),
      });
      this.inputAudioBuffer = ZPRealtimeUtils.mergeInt16Arrays(
   ...
   * @returns {Promise<{item: ZPItemType}>}
   */

...
/**
 * adoption guides： 
 * OpenAI: https://platform.openai.com/docs/api-reference/realtime-client-events/conversation/item/create
 *         https://platform.openai.com/docs/guides/realtime-model-capabilities#about-realtime-sessions
 * Zhipu:  https://bigmodel.cn/dev/api/rtav/GLM-Realtime
 */
 ...
 * @class
 */
export class ZPRealtimeConversation {
  ...
  EventProcessors = {
    'conversation.item.created': (event) => {
      ...
      if (newItem.type === 'message') {
        if (newItem.role === 'user') {
          ...
          //newItem.status = 'incomplete'; // zhipu: tbd
        }
    ...
    'response.audio.delta': (event) => {
      ...
      const arrayBuffer = ZPRealtimeUtils.base64ToArrayBuffer(delta);
      const appendValues = new Int16Array(arrayBuffer);
      item.formatted.audio = ZPRealtimeUtils.mergeInt16Arrays(
   ...
   * @param {Object} event
   * @param  {...any} args
   * @returns {item: import('./client.js').ZPItemType | null, delta: ItemContentDeltaType | null}
   */
   ...
   * @param {string} id
   * @returns {import('./client.js').ZPItemType}
   */
  getItem(id) {

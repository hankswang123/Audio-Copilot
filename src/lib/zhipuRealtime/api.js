...
export class ZPRealtimeAPI extends ZPRealtimeEventHandler {
  /**
  ...
  async connect({ model } = { model: 'gpt-4o-realtime-preview-2024-10-01' }) {
    ...
    if (globalThis.WebSocket) {
      ...
      ws.addEventListener('message', (event) => {
        ...
        // decode the rawdata as bytes to string
        const tmpJSONString = String.fromCharCode(...message.data);
        // Parse the JSON string into an object
        const zpEvent = JSON.parse(tmpJSONString);        

        this.receive(zpEvent.type, zpEvent);
      ...
      const zhipuURL = "wss://open.bigmodel.cn/api/paas/v4/realtime";
      const ws = new WebSocket(zhipuURL, {
        headers: {
          "Authorization": "Bearer " + process.env.ZHIPUAI_API_KEY,
        },
      });      
      ws.on('message', (data) => {
        console.log('Relay Server received from zhipu: ' + data);
        const message = JSON.parse(data.toString());
        console.log('Relay Server received from zhipu: ' + message);
        this.receive(message.type, message);
      });
      return new Promise((resolve, reject) => {
        const connectionErrorHandler = () => {
   ...
   */
  send(eventName, data) {
    this.log("send is called from server:" + data);
    if (!this.isConnected()) {
    ...
    const event = {
      event_id: ZPRealtimeUtils.generateId('evt_'),
      type: eventName,
      ...data,
    };
